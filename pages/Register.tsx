...
const Register: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      // Prompt Requirement: Hit real registration endpoint (if exists) or simulate real transmission
      // For now, we standardize the transmission logic to be ready for the hook integration
      showToast('Institutional registration packet transmitted for audit.', 'success');
      setIsSuccess(true);
    } catch (err) {
      showToast('Transmission failure: Infrastructure timeout.', 'error');
    }
  };
        ...
        <div className="flex justify-center">
          <div className="bg-slate-200/50 p-1.5 rounded-2xl flex gap-1 shadow-inner backdrop-blur-sm">
            <button
              ...
              className={`px-10 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'ORG' ? 'bg-white text-slate-950 shadow-md scale-100' : 'text-slate-400 hover:text-slate-600'
                }`}
            >
              Institutional
            </button>
            <button
              ...
              className={`px-10 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'DONOR' ? 'bg-white text-slate-950 shadow-md scale-100' : 'text-slate-400 hover:text-slate-600'
                }`}
            >
              Emergency Donor
            </button>
            ...
            <div className="bg-slate-50 p-8 border-b border-slate-100 flex justify-center gap-4">
              {([UserRole.HOSPITAL, UserRole.BLOOD_BANK] as const).map(role => (
                <button
                  ...
                  className={`px-10 py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all border-2 ${selectedRole === role
