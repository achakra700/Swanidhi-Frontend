...
import { useDonorProfile } from '../hooks/useDonor';
...
const DonorDashboard: React.FC = () => {
  ...
  // Unified requests state with persistence simulation
  const [requests, setRequests] = useState<DonationRequest[]>(() => {
    const saved = localStorage.getItem('ls_donor_reqs_mock');
    if (saved) return JSON.parse(saved);
    return [
      { id: 'REQ-101', hub: 'Sector-4 Hub', bloodType: 'B+', urgency: 'High', date: '2024-03-25', status: 'PENDING', units: 1 },
      { id: 'REQ-102', hub: 'Metro Reserve', bloodType: 'B+', urgency: 'Standard', date: '2024-03-28', status: 'PENDING', units: 1 },
      { id: 'REQ-098', hub: 'City Central', bloodType: 'B+', urgency: 'Standard', date: '2024-01-15', status: 'COMPLETED', units: 1 }
    ];
  });

  useEffect(() => {
    localStorage.setItem('ls_donor_reqs_mock', JSON.stringify(requests));
  }, [requests]);
          ...
          {[
            ...
            <button 
              ...
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                activeTab === v.id ? 'bg-white text-slate-900 shadow-sm border border-slate-200' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              {v.label}
           ...
           <table className="w-full text-left">
             <thead className="bg-slate-50 border-b border-slate-100">
               ...
               <tbody className="divide-y divide-slate-50">
                 {requests.filter(r => r.status === 'COMPLETED' || r.status === 'DECLINED').length === 0 ? (
                   <tr>
                   ...
                   requests.filter(r => r.status === 'COMPLETED' || r.status === 'DECLINED').map((log, i) => (
